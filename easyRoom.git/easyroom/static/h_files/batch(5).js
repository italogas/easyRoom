...
try {
/* module-key = 'confluence.extra.cipe:cipe-tutorial', location = 'js/cipe-tutorial.js' */
(function ($) {

    window.InPlaceEditorTutorial = {

        template: null,

        show: function () {
            var t = this;
            t.template = Confluence.Templates.CIPE;
            var containerId = t.prepareTutorialMessageContainer();
            t.showTutorialMessage(containerId);
        },
        ...
        prepareTutorialMessageContainer: function () {
            var t = this;
            ...
            var tutorialContainer = $('<li/>', {
                'id': 'cipe-tutorial-container',
                'class': 'confluence-messages noteMessage'
            });

            if (t.isDocumentationTheme()) {
                $('#header-precursor').append(messageContainer);
            } else {
                $('#header-precursor').find('.cell').append(messageContainer);
            }

            messageContainer.append(tutorialContainer);
            return tutorialContainer.attr('id');
        },

        showTutorialMessage: function (containerId) {
            var t = this;
            var tutorialMsg = AJS.messages.success('#' + containerId, {
                body: "Find out the way how to conveniently edit long pages by parts. \u003ca id=\"tutorial-link\" href=\"#\">Read more\u003c\/a>",
                id: 'cipe-tutorial-message'
            });

            AJS.bind('messageClose', function (e) {
                if (e.target.id == tutorialMsg.attr('id')) {
                    t.doNotShowTutorial();
                }
            });
        ...
        doNotShowTutorial: function () {
            $.ajax({
                cache: false,
                type: 'GET',
                contentType: 'application/json; charset=utf-8',
                url: AJS.params.contextPath + '/rest/inplaceeditor/1.0/tutorial/hide',
                data: {
        ...
        setPrimaryButton: function (dialog, title) {
            dialog.addButton(title, function (dialog) {
                dialog.nextPage();
            }, 'aui-button aui-button-primary tutorial-primary-button');
            $('.tutorial-primary-button').removeClass('button-panel-button');
        },

        setBackButton: function (dialog) {
            dialog.addButton("Back", function (dialog) {
                dialog.prevPage();
            });
        },

        setCancelLink: function (dialog) {
            dialog.addCancel("Close", function (dialog) {
                dialog.remove();
            });
        },

        addTutorialHomePage: function (dialog) {
            var t = this;
            dialog.addHeader("What is InPlace Editor?");
            dialog.addPanel('', t.template.tutorialHomePage(), '');
            t.setPrimaryButton(dialog, "View Tutorial");
            t.setCancelLink(dialog);
        },

        addTutorialStepOne: function (dialog) {
            var t = this;
            dialog.addPage();
            dialog.addHeader("Sectional Editing");
            dialog.addPanel('', t.template.tutorialStepOnePage(), '');
            t.setBackButton(dialog);
            t.setPrimaryButton(dialog, "Next");
            t.setCancelLink(dialog);
        },

        addTutorialStepTwo: function (dialog) {
            var t = this;
            dialog.addPage();
            dialog.addHeader("Native editor");
            dialog.addPanel('', t.template.tutorialStepTwoPage(), '');
            t.setBackButton(dialog);
            t.setPrimaryButton(dialog, "Next");
            t.setCancelLink(dialog);
        },

        addTutorialStepThree: function (dialog) {
            var t = this;
            dialog.addPage();
            dialog.addHeader("Easy configuration");
            dialog.addPanel('', t.template.tutorialStepThreePage(), '');
            t.setBackButton(dialog);
            t.setCancelLink(dialog);
        },
        ...
        openTutorial: function () {
            ...
            t.addTutorialHomePage(dialog);
            t.addTutorialStepOne(dialog);
            t.addTutorialStepTwo(dialog);
            t.addTutorialStepThree(dialog);
    ...
    AJS.toInit(function () {
        AJS.Meta.getBoolean('show-cipe-tutorial') && InPlaceEditorTutorial.show();
    });
...
try {
/* module-key = 'confluence.extra.cipe:cipe-tutorial', location = '/templates/soy/tutorial.soy' */
// This file was automatically generated from tutorial.soy.
// Please don't edit this file by hand.

if (typeof Confluence == 'undefined') { var Confluence = {}; }
if (typeof Confluence.Templates == 'undefined') { Confluence.Templates = {}; }
if (typeof Confluence.Templates.CIPE == 'undefined') { Confluence.Templates.CIPE = {}; }


Confluence.Templates.CIPE.tutorialHomePage = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="aui-group"><div class="aui-item"><div class="tutorial-step1-small tutorial-img"/><div>', "Clicking the Pencil icon next to the heading allows you to  launch sectional editing.", '</div></div><div class="aui-item"><div class="tutorial-step2-small tutorial-img"/><div>', "Edit paragraphs in the native Confluence editor that opens in the pop-up form.", '</div></div><div class="aui-item"><div class="tutorial-step3-small tutorial-img"/><div>', "Activates only for selected spaces and heading levels and can be disabled at once.", '</div></div></div><div class="aui-message"><p class="title"><span class="aui-icon icon-generic"></span><strong>', "Attention", '</strong></p><p>', "If the Pencil icon is not displayed, ask your Confluence administrator to enable the add-on for the required space or heading level.", '</p></div>');
  return opt_sb ? '' : output.toString();
};


Confluence.Templates.CIPE.tutorialStepOnePage = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="aui-group"><div class="aui-item" style="width: 31%"><p>', "1. Instantly edit page parts with a handy \x3cb\x3esectional editing\x3c/b\x3e feature.", '</p><p>', "2. Save a plenty of time for other activities.", '</p><p>', "3. Be \x3cb\x3eefficient\x3c/b\x3e and get \x3cb\x3epleasure\x3c/b\x3e from in-place editing.", '</p></div><div class="aui-item"><div class="tutorial-step1-img"/><div></div>');
  return opt_sb ? '' : output.toString();
};


Confluence.Templates.CIPE.tutorialStepTwoPage = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="aui-group"><div class="aui-item" style="width: 23%"><p>', "1. \x3cb\x3eNative\x3c/b\x3e Confluence editor in the pop-up form.", '</p><p>', "2. Support for almost all \x3cb\x3efeatures of standard editor\x3c/b\x3e.", '</p><p>', "3. \x3cb\x3eInsert new sections\x3c/b\x3e before or after the part you are currently editing.", '</p></div><div class="aui-item"><div class="tutorial-step2-img"/><div></div>');
  return opt_sb ? '' : output.toString();
};


Confluence.Templates.CIPE.tutorialStepThreePage = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="aui-group"><div class="aui-item" style="width: 30%"><p>', "1. Activate add-on only for \x3cb\x3especific heading levels\x3c/b\x3e.", '</p><p>', "2. Text snippets without heading at the top of the page are automatically treated as a section.", '</p><p>', "3. \x3cb\x3eTemporarily disable\x3c/b\x3e add-on just to better see convenience of sectional editing.", '</p></div><div class="aui-item"><div class="tutorial-step3-img"/><div></div>');
  return opt_sb ? '' : output.toString();
};
