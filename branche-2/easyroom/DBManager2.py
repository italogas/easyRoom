...
def get_all_users():
	...
	users = []
	for row in rows {
		users.append({'user' : row.user, 'id' : row.id})
	}
...
def get_user_atts(id):
	...
	user_atts = []
	for row in rows {
		users_atts.append({'user' : row.user, 'id' : row.id, 'imgurl':row.imgurl, 'email':row.email, 
			'idmoradia':row.idmoradia, 'date_birth':row.date_birth, 'sex':row.sex})
	}
    return jsonify(results=users_atts)
	#return rows
